...
while read -r line ; do
  case $line in
    SYS*)
      ...
      # bat
      bat="%{F${cpu_cicon}}${sep_l_left} %{T2}${icon_mem}%{F${cpu_cfore} T1} ${sys_arr[8]}%"
      # wlan
  ...
  printf "%s\n" "%{l}${wsp}${title} %{r}${cpu}${stab}${mem}${stab}${bat}${stab}${diskr}${stab}${diskh}${stab}${wland}${stab}${wlanu}${stab}${ethd}${stab}${ethu}${stab}${vol}${stab}${date}${stab}${time}"
done
